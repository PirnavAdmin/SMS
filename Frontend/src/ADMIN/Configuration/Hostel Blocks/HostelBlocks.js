...
const initialBlocks = [
  { id: 1, blockId: "HB001", name: "Boys Hostel A", floors: "4", rooms: "80", capacity: "320", warden: "Mr. Kumar", status: "Active" },
  { id: 2, blockId: "HB002", name: "Girls Hostel A", floors: "3", rooms: "60", capacity: "240", warden: "Mrs. Priya", status: "Active" },
];
...
function HostelIcon({ type }) {
  const paths = {
    ...
    rooms: <><path d="M3 21h18M5 21V7l7-4 7 4v14M9 10h2v2H9zM15 10h2v2h-2zM9 15h2v2H9zM15 15h2v2h-2z" /></>,
...
export default function HostelBlocks() {
  ...
  const filteredBlocks = useMemo(() => {
    ...
    return query ? blocks.filter((block) => `${block.blockId} ${block.name} ${block.floors} ${block.rooms} ${block.capacity} ${block.warden} ${block.status}`.toLowerCase().includes(query)) : blocks;
  }, [blocks, search]);
  ...
  const handleSubmit = (event) => {
    ...
    const values = { blockId: formData.blockId.trim().toUpperCase(), name: formData.name.trim(), floors: formData.floors, rooms: formData.rooms, capacity: formData.capacity, warden: formData.warden.trim(), status: formData.active ? "Active" : "Inactive" };
    if (editingId !== null) setBlocks((current) => current.map((block) => block.id === editingId ? { ...block, ...values } : block));
    else setBlocks((current) => [...current, { id: Date.now(), ...values }]);
    resetForm();
  };
  const handleEdit = (block) => { setFormData({ blockId: block.blockId, name: block.name, floors: block.floors, rooms: block.rooms, capacity: block.capacity, warden: block.warden, active: block.status === "Active" }); setEditingId(block.id); };
  const handleDelete = (id) => { setBlocks((current) => current.filter((block) => block.id !== id)); if (editingId === id) resetForm(); };

  return <div className="hb-page">
    <section className="hb-card hb-form-card">
      <div className="hb-watermark" aria-hidden="true"><span className="hb-watermark-building"><i /><i /><i /><i /><i /><i /></span><span className="hb-watermark-bed"><i /></span></div>
      <div className="hb-section-title"><span className="hb-heading-icon"><HostelIcon type="building" /></span><h3>Configuration / Hostel Blocks</h3></div>
      <form className="hb-form" onSubmit={handleSubmit}>
        <label className="hb-field"><span>Block ID *</span><input name="blockId" value={formData.blockId} onChange={handleChange} placeholder="e.g., HB003" required /></label>
        <label className="hb-field"><span>Block Name *</span><input name="name" value={formData.name} onChange={handleChange} placeholder="Enter Block Name" required /></label>
        <label className="hb-field"><span>Number of Floors *</span><input type="number" min="1" name="floors" value={formData.floors} onChange={handleChange} placeholder="Enter Floors" required /></label>
        <label className="hb-field"><span>Number of Rooms *</span><input type="number" min="1" name="rooms" value={formData.rooms} onChange={handleChange} placeholder="Enter Rooms" required /></label>
        <label className="hb-field"><span>Total Capacity *</span><input type="number" min="1" name="capacity" value={formData.capacity} onChange={handleChange} placeholder="Enter Capacity" required /></label>
        <label className="hb-field"><span>Warden Name *</span><input name="warden" value={formData.warden} onChange={handleChange} placeholder="Enter Warden Name" required /></label>
        <label className="hb-checkbox-field"><input type="checkbox" name="active" checked={formData.active} onChange={handleChange} /><span>Is Active?</span></label>
        <div className="hb-form-actions"><button className="hb-save-button" type="submit"><HostelIcon type="save" />{editingId !== null ? "Update Block" : "Save Block"}</button><button className="hb-clear-button" type="button" onClick={resetForm}><HostelIcon type="clear" />Clear</button></div>
      </form>
    </section>
    <section className="hb-card hb-details-card">
      <div className="hb-section-title"><span className="hb-heading-icon"><HostelIcon type="rooms" /></span><h3>Hostel Block Details</h3></div>
      <div className="hb-table-tools"><label className="hb-show-control">Show <select value={entries} onChange={(event) => { setEntries(event.target.value); setCurrentPage(1); }}><option value="10">10</option><option value="25">25</option><option value="50">50</option><option value="100">100</option></select> entries</label><label className="hb-search-control">Search: <input placeholder="Search..." value={search} onChange={(event) => { setSearch(event.target.value); setCurrentPage(1); }} /></label></div>
      <div className="hb-table-wrap"><table className="hb-table"><thead><tr><th>Block ID</th><th>Block Name</th><th>Floors</th><th>Rooms</th><th>Capacity</th><th>Warden</th><th>Status</th><th className="hb-action-column">Action</th></tr></thead><tbody>{visibleBlocks.length ? visibleBlocks.map((block) => <tr key={block.id}><td>{block.blockId}</td><td>{block.name}</td><td>{block.floors}</td><td>{block.rooms}</td><td>{block.capacity}</td><td>{block.warden}</td><td><span className={block.status === "Active" ? "hb-status-active" : "hb-status-inactive"}>{block.status}</span></td><td><div className="hb-action-buttons"><button className="hb-edit-button" type="button" onClick={() => handleEdit(block)}><HostelIcon type="edit" />Edit</button><button className="hb-delete-button" type="button" onClick={() => handleDelete(block.id)}><HostelIcon type="delete" />Delete</button></div></td></tr>) : <tr><td className="hb-empty" colSpan="8">No hostel blocks found.</td></tr>}</tbody></table></div>
      <div className="hb-pagination-bar"><p>Showing {firstEntry} to {lastEntry} of {filteredBlocks.length} entries</p><div className="hb-pagination-actions"><button className="hb-page-button" type="button" disabled={currentPage <= 1} onClick={() => setCurrentPage((page) => page - 1)}>Prev</button><button className="hb-page-button hb-page-current" type="button">{currentPage}</button><button className="hb-page-button" type="button" disabled={currentPage >= totalPages} onClick={() => setCurrentPage((page) => page + 1)}>Next</button></div></div>
    </section>
  </div>;
}
