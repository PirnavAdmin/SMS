...
export default function AddUniformFee({ onSave, editData, onCancelEdit }) {
  ...
  return <section className="uf-card uf-form-card">
    ...
    <form className="uf-form" onSubmit={submit}>
      <label className="uf-field"><span>Academic Year <b className="uf-required">*</b></span><select value={form.academicYear} onChange={(e) => update("academicYear", e.target.value)}><option>2024-2025</option><option>2025-2026</option></select>{errors.academicYear && <small>{errors.academicYear}</small>}</label>
      <label className="uf-field"><span>Uniform Name <b className="uf-required">*</b></span><select value={form.uniformName} onChange={(e) => update("uniformName", e.target.value)}><option value="">Select Name</option><option>Boys Dongri</option><option>Girls Dongri</option></select>{errors.uniformName && <small>{errors.uniformName}</small>}</label>
      <label className="uf-field"><span>Size / Quantity <b className="uf-required">*</b></span><select value={form.size} onChange={(e) => update("size", e.target.value)}><option value="">Select Size</option><option>10</option><option>12</option><option>14</option></select>{errors.size && <small>{errors.size}</small>}</label>
      <label className="uf-field"><span>Fee <b className="uf-required">*</b></span><input type="number" min="0" value={form.fee} onChange={(e) => update("fee", e.target.value)} placeholder="Enter Fee" />{errors.fee && <small>{errors.fee}</small>}</label>
