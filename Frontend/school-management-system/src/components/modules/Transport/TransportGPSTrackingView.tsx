...
export const TransportGPSTrackingView: React.FC = () => {
  ...
  const [selectedVehicle, setSelectedVehicle] = useState(
    vehicleMasters[0]?.vehicleNumber || "BUS-101",
  );
    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      <div>
        <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2">
          <Navigation className="w-6 h-6 text-sky-500 animate-spin" /> GPS
          Telematics & Live Location Tracker
        </h2>
        <p className="text-xs text-slate-500">
          Real-time GPS vehicle telematics, route waypoints, and speed
          simulation feed
        </p>
      </div>
              ...
              >
                <div className="text-left">
                  ...
                  <p className="text-[10px] opacity-80">
                    {v.vehicleType} • GPS: {v.gpsDeviceId}
                  </p>
                </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div>
                <h3 className="font-extrabold text-base text-slate-900 dark:text-white flex items-center gap-2">
                  Live Tracking - {selectedVehicle}
                </h3>
                <p className="text-xs text-slate-500">
                  Speed: 42 km/h • Heading: East Suburbs Expressway
                </p>
              </div>
              ...
              <div className="relative text-center space-y-3">
                ...
                <div>
                  <p className="text-sm font-black tracking-wide">
                    {selectedVehicle} En Route
                  </p>
                  <p className="text-xs text-sky-300">
                    Approaching Stop #3: JNTU Metro Gate (Eta: 4 Mins)
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 flex items-center gap-3">
            ...
            <div className="text-xs">
              <p className="font-extrabold text-slate-900 dark:text-white">
                Third-Party GPS Hardware API Integration Section
              </p>
              <p className="text-slate-500">
                Supports AIS-140 GPS Devices, MapmyIndia, Traccar, Google Maps
                Matrix, and Parent Mobile App push notifications.
              </p>
            </div>
          </div>
        </div>
