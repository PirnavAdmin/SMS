...
import { Percent, Plus, Search, Edit, Trash2, UserPlus, Filter, GraduationCap, Sparkles, CheckCircle2 } from 'lucide-react';
...
export const DiscountsView: React.FC = () => {
  const { discounts, studentDiscounts, students, addDiscount, updateDiscount, deleteDiscount, assignDiscountToStudent, removeStudentDiscount, academicClasses } = useData();
  ...
  const filteredAwardedList = studentDiscounts.filter(sd => {
    ...
    const matchesClass = allocatedClassFilter === 'All' || (st && st.className === allocatedClassFilter);
  ...
  const handleAllocSubmit = (e: React.SyntheticEvent) => {
    ...
    if (!allocStudentId || !allocDiscountId) {
      addToast('warning', 'Selection Required', 'Select both student and discount rule.');
      return;
    }
    assignDiscountToStudent(allocStudentId, allocDiscountId);
    addToast('success', 'Concession Granted', 'Applied concession to student successfully.');
      ...
      <div className="glass-card p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              type="text"
              placeholder={activeTab === 'master' ? "Search discount name..." : "Search student or concession..."}
              value={query}
      ...
      {activeTab === 'master' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredDiscounts.map(d => (
            <div key={d.id} className="glass-card p-5 rounded-2xl flex items-center justify-between hover:shadow-md transition-shadow">
              <div>
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => handleOpenEdit(d)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-sky-600"><Edit className="w-4 h-4" /></button>
                <button onClick={() => setDeletingDisc(d)} className="p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/40 text-rose-600"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
        </div>
                ...
                {filteredAwardedList.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-400">
                      No student concessions granted yet. Click "+ Grant Student Concession" to apply a rule.
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      <tr key={sd.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                        <td className="py-3 px-4 font-bold text-slate-900 dark:text-white flex items-center gap-2">
                          ...
                          {st?.className && (
                            <span className="text-[10px] font-semibold text-slate-400">({st.className}-{st.section})</span>
                          )}
                        </td>
                        <td className="py-3 px-4 font-semibold text-sky-600 dark:text-sky-400">{sd.discountName}</td>
                        <td className="py-3 px-4 font-bold text-emerald-600 dark:text-emerald-400">
                          {disc ? (disc.mode === 'Percentage' ? `${disc.value}% Waiver` : `${formatCurrency(disc.value)} Off`) : 'Active'}
                        </td>
                        ...
                        <td className="py-3 px-4 text-right">
                          <button
                            onClick={() => {
                              ...
                              addToast('info', 'Concession Removed', 'Removed concession record.');
                            }}
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border outline-none font-medium" />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={formData.mode} onChange={e => setFormData({ ...formData, mode: e.target.value as any })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border outline-none font-medium">
              ...
              <div>
                ...
                <input type="number" required value={formData.value} onChange={e => setFormData({ ...formData, value: Number(e.target.value) })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border font-bold text-emerald-600 outline-none" />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <button onClick={() => setIsAllocOpen(false)} className="text-slate-400 hover:text-slate-600">✕</button>
            </div>
            ...
            <form onSubmit={handleAllocSubmit} className="space-y-4 text-xs">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[11px] font-bold text-sky-600 dark:text-sky-400">
                    {filteredStudentsForGrant.length} student{filteredStudentsForGrant.length !== 1 ? 's' : ''} available
                  </span>
                </div>
                <select
                  ...
                  required
                >
                  <option value="">-- Choose Student ({filteredStudentsForGrant.length} matching) --</option>
              ...
              <div>
                <label className="block font-bold text-slate-800 dark:text-slate-200 mb-1">
                  Select Discount Rule <span className="text-rose-500 font-bold ml-0.5">*</span>
                </label>
                <select
                ...
                >
                  {discounts.map(d => (
                    <option key={d.id} value={d.id}>
                      {d.name} ({d.mode === 'Percentage' ? `${d.value}% Concession` : `${formatCurrency(d.value)} Flat Off`})
                    </option>
                  ))}
                </select>
              </div>
