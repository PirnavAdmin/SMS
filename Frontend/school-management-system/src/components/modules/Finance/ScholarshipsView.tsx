...
import { Gift, Plus, Search, Edit, Trash2, CheckCircle2, UserPlus, X, Filter, GraduationCap, Award } from 'lucide-react';
...
export const ScholarshipsView: React.FC = () => {
  ...
  const [allocatedClassFilter, setAllocatedClassFilter] = useState<string>('All');
  ...
  const sortedClasses = useMemo(() => {
    return [...academicClasses].sort((a, b) => compareClassesAscending(a.name, b.name));
  }, [academicClasses]);

  // Filtered Students for the Award Modal
  const filteredStudentsForAward = useMemo(() => {
    return students.filter(st => {
      const matchesClass = allocClassFilter === 'All' || st.className === allocClassFilter;
  ...
  const filteredAwardedList = studentScholarships.filter(ss => {
    ...
    const matchesClass = allocatedClassFilter === 'All' || (ss as any).className === allocatedClassFilter;
  ...
  const handleAllocSubmit = (e: React.SyntheticEvent) => {
    ...
    addToast('success', 'Scholarship Awarded', 'Awarded scholarship to student successfully.');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-md shadow-emerald-500/20 flex items-center gap-1.5 transition-all"
          >
      ...
      <div className="glass-card p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              type="text"
              placeholder={activeTab === 'master' ? "Search scholarship name..." : "Search student or scholarship..."}
              value={query}
      ...
      {activeTab === 'master' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredScholarships.map(s => (
            <div key={s.id} className="glass-card p-5 rounded-2xl space-y-3 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => handleOpenEdit(s)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-sky-600"><Edit className="w-4 h-4" /></button>
                  <button onClick={() => setDeletingSch(s)} className="p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/40 text-rose-600"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
                ...
                {filteredAwardedList.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-400">
                      No student scholarships found. Click "+ Award Student Scholarship" to award a grant.
                    </td>
                  </tr>
                ) : (
                      ...
                      <td className="py-3 px-4 text-right">
                        <button
                          onClick={() => {
                            ...
                            addToast('info', 'Scholarship Revoked', `Revoked scholarship for ${ss.studentName}`);
                          }}
            ...
            <form onSubmit={handleSubmit} className="space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border outline-none" />
                </div>
                <div>
                  <label className="block font-semibold mb-1">Scholarship Code <span className="text-rose-500 font-bold ml-0.5">*</span></label>
                  <input type="text" required value={formData.code} onChange={e => setFormData({ ...formData, code: e.target.value })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border font-mono outline-none" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value as any })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border outline-none font-medium">
                    {SCHOLARSHIP_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block font-semibold mb-1">Discount Mode</label>
                  <select value={formData.discountType} onChange={e => setFormData({ ...formData, discountType: e.target.value as any })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border outline-none font-medium">
              ...
              {formData.discountType === 'Percentage' ? (
                <div><label className="block font-semibold mb-1">Percentage Waiver (%)</label><input type="number" value={formData.percentage} onChange={e => setFormData({ ...formData, percentage: Number(e.target.value) })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border font-bold text-emerald-600 outline-none" /></div>
              ) : (
                <div><label className="block font-semibold mb-1">Fixed Grant Amount (₹)</label><input type="number" value={formData.fixedAmount} onChange={e => setFormData({ ...formData, fixedAmount: Number(e.target.value) })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border font-bold text-emerald-600 outline-none" /></div>
              )}

              <div>
                <label className="block font-semibold mb-1">Eligibility Criteria</label>
                <input type="text" value={formData.eligibility} onChange={e => setFormData({ ...formData, eligibility: e.target.value })} className="w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800 border outline-none" />
              </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">Filter students by class grade or search by name & admission no</p>
                </div>
              </div>
            ...
            <form onSubmit={handleAllocSubmit} className="space-y-4 text-xs">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-[11px] font-bold text-sky-600 dark:text-sky-400">
                    {filteredStudentsForAward.length} student{filteredStudentsForAward.length !== 1 ? 's' : ''} available
                  </span>
                </div>
                <select
                  ...
                  required
                >
                  <option value="">-- Choose Student ({filteredStudentsForAward.length} matching) --</option>
                  {filteredStudentsForAward.map(st => (
                    <option key={st.id} value={st.id}>
                      {st.firstName} {st.lastName} ({st.className}{st.section ? `-${st.section}` : ''} • Adm: {st.admissionNo || st.id})
                    </option>
                  ))}
                </select>
              </div>
                ...
                <div className="p-3 rounded-xl bg-sky-50 dark:bg-sky-950/30 border border-sky-200 dark:border-sky-800/50 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-[11px] text-slate-500">
                      Class: <span className="font-semibold text-slate-700 dark:text-slate-300">{selectedStudentDetails.className}-{selectedStudentDetails.section}</span> • Adm No: <span className="font-semibold text-slate-700 dark:text-slate-300">{selectedStudentDetails.admissionNo}</span>
                    </div>
                  </div>
              ...
              <div>
                <label className="block font-bold text-slate-800 dark:text-slate-200 mb-1">
                  Select Scholarship Scheme <span className="text-rose-500 font-bold ml-0.5">*</span>
                </label>
                <select
                ...
                >
                  {scholarships.map(s => (
                    <option key={s.id} value={s.id}>
                      {s.name} ({s.discountType === 'Percentage' ? `${s.percentage}% Fee Waiver` : `${formatCurrency(s.fixedAmount || 0)} Grant`} • {s.type})
                    </option>
                  ))}
                </select>
              </div>
