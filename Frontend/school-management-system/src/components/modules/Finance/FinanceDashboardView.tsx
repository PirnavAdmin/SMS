...
export const FinanceDashboardView: React.FC = () => {
  ...
  const uniformCollection = studentUniformIssues
    ...
    .reduce((sum, issue) => {
      const uItem = uniforms.find(
        (u) =>
          u.id === issue.itemId ||
          u.category.toLowerCase() === issue.itemName.toLowerCase(),
      );
  ...
  const totalPending = students.reduce(
    (acc, s) => acc + getStudentFeeOutstandingSummary(s.id).totalOutstanding,
    0,
  );
  const totalExpected = totalCollected + totalPending;

  const todayStr = new Date().toISOString().split("T")[0];
  const todaysCollection = feePayments
    .filter((p) => p.paymentDate === todayStr)
    .reduce((acc, p) => acc + p.amountPaid, 0);

  const transportCollection = studentTransports.reduce(
    (acc, t) => acc + t.feeAmount,
    0,
  );
  const hostelCollection = studentHostels.reduce(
    (acc, h) => acc + h.feeAmount,
    0,
  );
  const scholarshipAmount = studentScholarships.reduce(
    (acc, s) =>
      acc + (s.discountType === "Percentage" ? 3750 : s.discountValue),
    0,
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 rounded-3xl space-y-4">
          ...
          <div className="space-y-3">
            {academicClasses.map((c) => {
              ...
              const classPaid = classStudents.reduce(
                (acc, s) => acc + s.paidFee,
                0,
              );
              const classTotal =
                classStudents.reduce((acc, s) => acc + s.totalFee, 0) || 1;
              const pct = Math.min(
                100,
                Math.round((classPaid / classTotal) * 100),
              );
                ...
                <div key={c.id} className="space-y-1">
                  <div className="flex justify-between text-xs font-bold">
                    ...
                    <span className="text-slate-500">
                      {formatCurrency(classPaid)} / {formatCurrency(classTotal)}{" "}
                      ({pct}%)
                    </span>
                  </div>
        ...
        <div className="glass-card p-6 rounded-3xl space-y-4">
          <h3 className="font-bold text-base text-slate-900 dark:text-white flex items-center gap-2">
            <PieChart className="w-5 h-5 text-sky-500" /> Fee Collection by
            Category
          </h3>
          <div className="space-y-3">
