...
import { getRoomTypes, createRoomType, updateRoomType, deleteRoomType, RoomType } from '../../../api/hostel';
...
export const RoomTypeMasterView: React.FC = () => {
  ...
  const fetchRoomTypes = useCallback(async () => {
    try {
      ...
      addToast('error', 'Failed to load room types', error.message);
    } finally {
      setLoading(false);
    }
  }, [addToast]);
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    const trimmedName = (form.roomTypeSpecification || '').trim();
    ...
    const capValue = Number(form.bedCapacity);
    ...
    try {
      ...
      const payload = {
        ...form,
        roomTypeSpecification: trimmedName,
        bedCapacity: capValue,
      };

      if (editingRt) {
        await updateRoomType(editingRt.roomTypeId, payload);
      ...
      addToast('error', editingRt ? 'Update Failed' : 'Creation Failed', error.message);
    } finally {
  ...
  const handleDelete = async () => {
    if (deletingRt) {
      try {
        await deleteRoomType(deletingRt.roomTypeId);
        addToast('success', 'Room Type Deleted', 'Room Type deleted successfully.');
        fetchRoomTypes();
      } catch (error: any) {
  ...
  const handleExportCSV = () => {
    ...
    const rows = roomTypes
      .map(
        rt =>
          `"${rt.roomTypeSpecification}",${rt.bedCapacity},"${rt.acType}","${rt.description || ''}","${rt.status}"`
      )
      .join('\n');
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium">
              {loading ? (
                 <tr>
                 <td colSpan={6} className="py-8 text-center text-slate-400">Loading room types...</td>
               </tr>
              ) : filtered.length === 0 ? (
                <tr>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  value={form.roomTypeSpecification || ''}
                  onChange={e => setForm({ ...form, roomTypeSpecification: e.target.value })}
