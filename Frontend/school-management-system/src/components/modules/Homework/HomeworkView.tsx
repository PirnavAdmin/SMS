...
export const HomeworkView: React.FC = () => {
  ...
  const handleAddAttachment = () => {
    if (newAttName && newAttName.trim()) {
      const trimmed = newAttName.trim();
      const att: HomeworkAttachment = {
        id: 'ATT-' + Math.floor(1000 + Math.random() * 9000),
        name: trimmed,
        url: '#',
        type: trimmed.toLowerCase().endsWith('.pdf')
          ? 'PDF'
          : trimmed.toLowerCase().match(/\.(png|jpg|jpeg|gif|webp)$/)
          ? 'Image'
          : 'Doc'
      };
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newItems: HomeworkAttachment[] = [];
    Array.from(files).forEach(file => {
      const ext = file.name.split('.').pop()?.toLowerCase() || '';
      let fileType = 'Doc';
      if (['pdf'].includes(ext)) fileType = 'PDF';
      else if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg'].includes(ext)) fileType = 'Image';
      else if (['doc', 'docx', 'txt', 'rtf', 'xlsx', 'ppt', 'pptx'].includes(ext)) fileType = 'Doc';
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-800 outline-none font-medium"
                  />
                  <button
                    type="button"
                    onClick={handleAddAttachment}
                    className="px-4 py-1.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-black cursor-pointer shadow-xs transition-colors"
                  >
                    Add
                  </button>
                </div>
                {attachments.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 pt-2">
                    {attachments.map(att => (
                      <span key={att.id} className="px-2.5 py-1 rounded-xl bg-sky-50 dark:bg-sky-950/60 border border-sky-200 dark:border-sky-800/80 text-sky-800 dark:text-sky-300 font-extrabold text-[11px] flex items-center gap-1.5 shadow-2xs">
                        <FileText className="w-3 h-3 text-sky-600" />
                        <span className="truncate max-w-[200px]">{att.name}</span>
                        <span className="text-[9px] px-1 py-0.2 rounded bg-sky-200/60 dark:bg-sky-800/60 text-sky-900 dark:text-sky-100 font-mono uppercase">{att.type}</span>
                        <button
                          type="button"
