...
export const PromoteStudentModal: React.FC<PromoteStudentModalProps> = ({
  ...
  const isHighestClass = student.className === highestClass;
  ...
  const handleSubmit = (e: React.SyntheticEvent) => {
    ...
    if (isHighestClass) {
      completeStudent(student.id, targetYear, 'Unknown');
      addToast('success', 'Graduated to Alumni', `Completed & graduated ${student.firstName} ${student.lastName} to Alumni.`);
    } else {
      promoteStudent(
        student.id,
        targetClass,
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            <div className={`p-2 rounded-xl ${isHighestClass ? 'bg-amber-50 text-amber-600 dark:bg-amber-950' : 'bg-brand-50 text-brand-600 dark:bg-brand-950'}`}>
              {isHighestClass ? <GraduationCap className="w-5 h-5" /> : <ArrowUpRight className="w-5 h-5" />}
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                {isHighestClass ? 'Complete Education / Graduate' : 'Promote Student'}
              </h3>
        ...
        {isHighestClass && (
          <div className="p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 flex items-start gap-2.5 text-xs text-amber-900 dark:text-amber-200">
            <Award className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
            <div>
              <p className="font-bold">Highest Class Completion</p>
              <p className="text-[11px] text-amber-700 dark:text-amber-400 mt-0.5">
                {student.firstName} is currently in {student.className} (School's Highest Class). Confirming will complete their school education and automatically create an Alumni record.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              ...
              className={`px-5 py-2 font-bold text-white rounded-xl shadow-md ${
                isHighestClass ? 'bg-amber-600 hover:bg-amber-500' : 'bg-brand-600 hover:bg-brand-500'
              }`}
            >
              {isHighestClass ? 'Complete & Graduate to Alumni' : 'Confirm Promotion'}
            </button>
          </div>
        </form>
      </div>
