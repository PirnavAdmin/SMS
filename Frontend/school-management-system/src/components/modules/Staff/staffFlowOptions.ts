...
export function normalizeStaffType(category?: string): StaffType {
  ...
  if (category === 'Teaching Staff' || category === 'Teacher') return 'Teaching Staff';
...
export const departmentDesignationMap: Record<string, string[]> = {
  ...
  'Mathematics': ['Principal', 'Vice Principal', 'Head of Department (HOD)', 'PGT Teacher', 'TGT Teacher', 'PRT Teacher', 'Subject Teacher', 'Assistant Teacher'],
  'English': ['Principal', 'Vice Principal', 'Head of Department (HOD)', 'PGT Teacher', 'TGT Teacher', 'PRT Teacher', 'Subject Teacher', 'Assistant Teacher'],
  'Science': ['Principal', 'Vice Principal', 'Head of Department (HOD)', 'PGT Teacher', 'TGT Teacher', 'PRT Teacher', 'Subject Teacher', 'Assistant Teacher'],
  'Social Studies': ['Principal', 'Vice Principal', 'Head of Department (HOD)', 'PGT Teacher', 'TGT Teacher', 'PRT Teacher', 'Subject Teacher', 'Assistant Teacher'],
  'Telugu': ['Principal', 'Vice Principal', 'Head of Department (HOD)', 'PGT Teacher', 'TGT Teacher', 'PRT Teacher', 'Subject Teacher', 'Assistant Teacher'],
  'Hindi': ['Principal', 'Vice Principal', 'Head of Department (HOD)', 'PGT Teacher', 'TGT Teacher', 'PRT Teacher', 'Subject Teacher', 'Assistant Teacher'],
  'Computer Science': ['Principal', 'Vice Principal', 'Head of Department (HOD)', 'PGT Teacher', 'TGT Teacher', 'PRT Teacher', 'Subject Teacher', 'Assistant Teacher'],
  'Physical Education': ['Head of Department (HOD)', 'Physical Education Teacher', 'Assistant Teacher'],
  'Arts': ['Head of Department (HOD)', 'Art Teacher', 'Assistant Teacher'],
  'Music': ['Head of Department (HOD)', 'Music Teacher', 'Assistant Teacher']
};

export function isAcademicAssignmentVisible(staffTypeCategory?: string): boolean {
  return normalizeStaffType(staffTypeCategory) === 'Teaching Staff';
}

export function getDepartmentOptions(staffTypeCategory?: string, settingsDepts?: any[]): string[] {
  ...
  const teachingDeptNames = new Set([
    'Mathematics', 'Science', 'English', 'Social Science', 'Social Studies',
    'Languages', 'Telugu', 'Hindi', 'Computer Science', 'Computer Science / ICT',
    'Commerce', 'Humanities', 'Fine Arts', 'Performing Arts', 'Arts', 'Music',
    'Physical Education', 'Pre-Primary', 'Special Education'
  ]);

  const nonTeachingDeptNames = new Set([
    'Accounts', 'Administration', 'Library', 'Laboratory', 'Transport',
    'Hostel', 'Reception', 'IT Support', 'HR', 'Admissions',
    'Housekeeping', 'Security', 'Maintenance', 'Medical', 'Stores & Inventory'
  ]);
  ...
  if (settingsDepts && settingsDepts.length > 0) {
    customDepts = settingsDepts
      .filter(d => {
        ...
        const targetType = typeof d === 'object' ? (d.staffType || d.employeeCategory) : null;
  ...
  let combined = Array.from(new Set([...defaultList, ...customDepts])).filter(Boolean);
...
export function getDesignationOptions(
  ...
  const teachingDesignationNames = new Set([
    ...
    'PGT Teacher', 'TGT Teacher', 'PRT Teacher', 'Pre-Primary / Nursery Teacher',
    'Subject Teacher', 'Assistant Teacher', 'Physical Education Teacher',
    'Music Teacher', 'Art Teacher', 'Activity Teacher', 'Special Educator'
  ]);

  const nonTeachingDesignationNames = new Set([
    'Accountant', 'Senior Accountant', 'Finance Manager', 'Finance Executive', 'Cashier', 'Billing Clerk', 'Accounts Assistant', 'Bursar',
    'Librarian', 'Assistant Librarian', 'Library Attendant', 'Library Assistant', 'Resource Center Executive',
    'Lab Assistant', 'Lab Technician', 'Lab In-charge', 'Lab Attendant', 'Physics Lab Assistant', 'Chemistry Lab Assistant', 'Biology Lab Assistant', 'Computer Lab Assistant',
    'Receptionist', 'Front Desk Executive', 'Information Desk Officer', 'Telecaller / Helpdesk Executive',
    'Transport Coordinator', 'Fleet Supervisor', 'Driver', 'Bus Conductor', 'Vehicle Mechanic', 'Transport Manager', 'Cleaner / Helper',
    'Hostel Warden', 'Chief Warden', 'Assistant Warden', 'Hostel Caretaker', 'Mess Manager', 'Hostel Attendant',
    'IT Executive', 'Network Administrator', 'System Administrator', 'IT Support Specialist', 'Hardware Engineer', 'EDP Manager',
    'Office Assistant', 'Store Keeper', 'Administrator', 'Office Manager', 'Administrative Executive', 'HR Executive', 'HR Manager', 'Admission Officer', 'Admissions Counselor', 'Admissions Head',
    'Security Guard', 'Security Supervisor', 'Security Officer', 'Chief Security Officer', 'Head Guard', 'Gate Keeper', 'Night Guard',
    'Housekeeping Staff', 'Housekeeping Supervisor', 'Sweeper', 'Sanitation Worker', 'Cleaning Attendant', 'Head Housekeeper',
    'Electrician', 'Plumber', 'Gardener', 'Maintenance Mechanic', 'Facility Supervisor', 'Carpenter', 'Painter', 'Estate Officer', 'Attender'
  ]);
  ...
  if (settingsDesignations && settingsDesignations.length > 0) {
    customDesignations = settingsDesignations
      .filter(d => {
        ...
        const targetCategory = typeof d === 'object' ? (d.staffType || d.employeeCategory) : null;
        if (targetCategory && targetCategory !== 'Both' && targetCategory !== 'All' && normalizeStaffType(targetCategory) !== staffType) {
          return false;
        }

        const targetDept = typeof d === 'object' ? d.department : null;
        if (selectedDepartment && targetDept && targetDept !== selectedDepartment && targetDept !== 'All') {
          return false;
        }
  ...
  let combined = Array.from(new Set([...baseList, ...customDesignations])).filter(Boolean);

  if (staffType === 'Teaching Staff') {
    combined = combined.filter(desig => !nonTeachingDesignationNames.has(desig));
  } else {
...
export const defaultBasicStaffFormState = (category: string = 'Teaching Staff'): BasicStaffFormState => ({
...
export function calculateExperienceYearsMonths(fromDate: string, toDate: string): string {
  if (!fromDate || !toDate) return '0 Years 0 Months';
  const start = new Date(fromDate);
  const end = new Date(toDate);
  if (isNaN(start.getTime()) || isNaN(end.getTime()) || end < start) return '0 Years 0 Months';
...
export function getDocumentRequirements(category?: string) {
  return normalizeStaffType(category) === 'Teaching Staff' ? teachingDocumentRequirements : nonTeachingDocumentRequirements;
}
...
export function buildBasicStaffCreatePayload(form: BasicStaffFormState): Omit<Staff, 'id'> {
  ...
  const formattedQual = form.qualifications.length > 0
    ? form.qualifications.map(q => `${q.qualification} (${q.specialization || 'General'})`).join(', ')
    : '';
  ...
  if (form.experiences.length > 0) {
    form.experiences.forEach(exp => {
      if (exp.fromDate && exp.toDate) {
        const diffMs = new Date(exp.toDate).getTime() - new Date(exp.fromDate).getTime();
        if (diffMs > 0) {
  ...
  return {
    empId: form.empId,
    employeeCategory: (normalizedCategory === 'Teaching Staff' ? 'Teacher' : 'Staff') as any,
    branch: form.branch,
    ...
    avatar: form.photoUrl || 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=400&auto=format&fit=crop&q=80',
