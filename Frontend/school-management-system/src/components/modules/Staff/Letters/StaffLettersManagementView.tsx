...
export const StaffLettersManagementView: React.FC<StaffLettersManagementViewProps> = ({ onNavigate }) => {
  ...
  const [globalSignatoryName, setGlobalSignatoryName] = useState(schoolProfile?.principalName || 'Dr. K. S. Sharma');
  const [globalSignatoryTitle, setGlobalSignatoryTitle] = useState('Principal & Authorized Signatory');
  const [globalProbationMonths, setGlobalProbationMonths] = useState(6);
  ...
  return (
    <div className="space-y-3 max-w-7xl mx-auto pb-10 animate-in fade-in duration-300">
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (letterToDelete) {
            deleteStaffLetterRecord(letterToDelete.id);
            addToast('success', 'Letter Revoked', 'Issued letter record has been removed.');
