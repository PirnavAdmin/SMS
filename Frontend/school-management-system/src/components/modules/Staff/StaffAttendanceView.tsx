...
export const StaffAttendanceView: React.FC = () => {
  ...
  const isPersonalView = userRole === 'teacher' || userRole === 'class-teacher' || !canMarkAttendance;
  ...
  const [persCheckInTime, setPersCheckInTime] = useState<string | null>(() => localStorage.getItem('teacher_check_in_time'));
  const [persCheckOutTime, setPersCheckOutTime] = useState<string | null>(() => localStorage.getItem('teacher_check_out_time'));
  ...
  const [requests, setRequests] = useState([
    { id: 'REQ-1', date: '2026-07-20', type: 'Missed Check-In', status: 'Pending', reason: 'System outage at front gate scanner' },
    { id: 'REQ-2', date: '2026-07-12', type: 'Missed Check-Out', status: 'Approved', reason: 'Left early for official field trip' }
  ]);
  ...
  useEffect(() => {
    ...
    const calcHours = () => {
      ...
      const end = persCheckOutTime ? new Date(persCheckOutTime).getTime() : Date.now();
  ...
  const handlePersCheckIn = () => {
    ...
    addToast('success', 'Checked In Successfully', `Recorded check-in at ${new Date(nowIso).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`);
  };
  ...
  const handlePersCheckOut = () => {
    ...
    addToast('info', 'Checked Out Successfully', `Recorded check-out at ${new Date(nowIso).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`);
  };
  ...
  const lateByMins = useMemo(() => {
    ...
    const checkInParsed = new Date(persCheckInTime!);
    const checkInMin = checkInParsed.getHours() * 60 + checkInParsed.getMinutes();
  ...
  const fullHistory = useMemo(() => {
    const todayRecord = persCheckInTime ? {
      date: todayStr,
      checkIn: new Date(persCheckInTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      checkOut: persCheckOutTime ? new Date(persCheckOutTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--',
      workingHours: persWorkingHours,
      status: todayStatus
    } : null;

    const base = [
      { date: '2026-07-29', checkIn: '09:02 AM', checkOut: '--', workingHours: '3h 15m', status: 'Present' },
      { date: '2026-07-28', checkIn: '08:55 AM', checkOut: '05:32 PM', workingHours: '8h 37m', status: 'Present' },
      { date: '2026-07-27', checkIn: '09:15 AM', checkOut: '05:00 PM', workingHours: '7h 45m', status: 'Late' },
      { date: '2026-07-24', checkIn: '08:45 AM', checkOut: '04:30 PM', workingHours: '7h 45m', status: 'Present' },
      { date: '2026-07-23', checkIn: '08:52 AM', checkOut: '05:00 PM', workingHours: '8h 08m', status: 'Present' },
      { date: '2026-07-22', checkIn: '--', checkOut: '--', workingHours: '0h 0m', status: 'Leave' },
      { date: '2026-07-21', checkIn: '08:58 AM', checkOut: '04:35 PM', workingHours: '7h 37m', status: 'Present' },
      { date: '2026-07-20', checkIn: '--', checkOut: '--', workingHours: '0h 0m', status: 'Absent' }
    ];
  ...
  }, [persCheckInTime, persCheckOutTime, persWorkingHours, todayStatus, todayStr, personalFilterDate, personalFilterMonth, personalSearchQuery]);
  ...
  const handleApplyLeaveSubmit = (e: React.FormEvent) => {
    ...
    const daysCount = diffMs >= 0 ? Math.ceil(diffMs / (1000 * 60 * 60 * 24)) + 1 : 0;
    
    if (daysCount === 0) {
      addToast('error', 'Invalid Date Range', 'End date must be on or after start date.');
      return;
    }

    const typeKey = leaveType.toLowerCase() as 'casual' | 'sick' | 'paid';
    const available = typeKey === 'casual' ? leaveBalance.casual : typeKey === 'sick' ? leaveBalance.sick : leaveBalance.paid;
    
    if (available < daysCount) {
      addToast('error', 'Insufficient Leave Balance', `You only have ${available} days of ${leaveType} leave remaining.`);
      return;
    }

    setLeaveBalance(prev => ({
      ...prev,
      [typeKey]: (typeKey === 'casual' ? prev.casual : typeKey === 'sick' ? prev.sick : prev.paid) - daysCount
    }));

    addToast('success', 'Leave Applied Successfully', `Submitted application for ${daysCount} days of ${leaveType} Leave.`);
  ...
  const handleDownloadReport = () => {
    addToast('success', 'Report Export Started', 'Downloading personal attendance registry in Excel format.');
  };
        ...
        <div className="glass-card p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-slate-900 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-slate-505 font-bold">
              <span>👤 Name: <strong className="text-slate-800 dark:text-slate-200">{teacher.firstName} {teacher.lastName}</strong></span>
              <span>🆔 Emp ID: <strong className="text-slate-800 dark:text-slate-200">{teacher.empId || teacher.id}</strong></span>
              <span>🏢 Dept: <strong className="text-slate-800 dark:text-slate-200">{teacher.department}</strong></span>
              <span>📅 Date: <strong className="text-slate-800 dark:text-slate-200">{new Date().toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}</strong></span>
            </div>
          </div>

          <div className="flex flex-col items-end gap-1">
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Attendance Status Today</p>
            <span className={`px-3.5 py-1.5 rounded-full text-xs font-black uppercase tracking-wider ${
              todayStatus === 'Present' ? 'bg-emerald-100 text-emerald-805 dark:bg-emerald-950/60 dark:text-emerald-400' :
              todayStatus === 'Late' ? 'bg-amber-100 text-amber-805 dark:bg-amber-950/60 dark:text-amber-400' :
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="glass-card p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-slate-900 space-y-4 shadow-sm">
              <div className="flex items-center gap-2 pb-1 border-b border-slate-105 dark:border-slate-800/80">
                ...
                <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">Today's Attendance</h3>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 text-center space-y-0.5">
                  ...
                  <p className="font-mono font-black text-slate-850 dark:text-white text-sm">
                    {persCheckInTime ? new Date(persCheckInTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--'}
                  </p>
                </div>
                <div className="p-3 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 text-center space-y-0.5">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Check-Out</span>
                  <p className="font-mono font-black text-slate-850 dark:text-white text-sm">
                    {persCheckOutTime ? new Date(persCheckOutTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--'}
                  </p>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-500">Working Hours:</span>
                  <span className="font-mono font-black text-slate-800 dark:text-white">{persWorkingHours}</span>
                </div>
                <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-805 pt-2">
                  <span className="font-bold text-slate-500">Status today:</span>
                  <span className="font-black text-slate-800 dark:text-white">{todayStatus}</span>
                </div>
                {todayStatus === 'Late' && (
            ...
            <div className="glass-card p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-slate-900 space-y-4 shadow-sm">
              <div className="flex items-center gap-2 pb-1 border-b border-slate-105 dark:border-slate-800/80">
                ...
                <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">Monthly Summary</h3>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 flex flex-col justify-center">
                  ...
                  <span className="text-base font-black text-emerald-600 dark:text-emerald-400">20 Days</span>
                </div>
                <div className="p-3 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 flex flex-col justify-center">
                  <span className="text-[10px] text-slate-400 uppercase font-bold">Absent Days</span>
                  <span className="text-base font-black text-rose-600 dark:text-rose-400">0 Days</span>
                </div>
                <div className="p-3 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 flex flex-col justify-center">
                  <span className="text-[10px] text-slate-400 uppercase font-bold">Late Days</span>
                  <span className="text-base font-black text-amber-600 dark:text-amber-400">2 Days</span>
                </div>
                <div className="p-3 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 flex flex-col justify-center">
                  <span className="text-[10px] text-slate-400 uppercase font-bold">Leave Days</span>
                  <span className="text-base font-black text-sky-600 dark:text-sky-400">1 Day</span>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-slate-50/50 dark:bg-slate-900/30 rounded-2xl border border-slate-100 dark:border-slate-800/50 flex items-center justify-between text-xs">
                <span className="font-bold text-slate-500">Total Hours (Month):</span>
                <span className="font-black text-slate-850 dark:text-white">168 hrs</span>
              </div>
            </div>
            ...
            <div className="glass-card p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-slate-900 space-y-4 shadow-sm">
              <div className="flex items-center gap-2 pb-1 border-b border-slate-105 dark:border-slate-800/80">
                ...
                <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">Leave Balance</h3>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2.5">
                <div className="p-2.5 rounded-2xl bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100/50 dark:border-indigo-900/40 text-center space-y-0.5">
                  <span className="text-[9px] font-bold text-indigo-700 dark:text-indigo-400 uppercase">Casual</span>
                  <p className="font-black text-indigo-900 dark:text-white text-sm">{leaveBalance.casual}</p>
                </div>
                <div className="p-2.5 rounded-2xl bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100/50 dark:border-amber-900/40 text-center space-y-0.5">
                  <span className="text-[9px] font-bold text-amber-700 dark:text-amber-400 uppercase">Sick</span>
                  <p className="font-black text-amber-900 dark:text-white text-sm">{leaveBalance.sick}</p>
                </div>
                <div className="p-2.5 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100/50 dark:border-emerald-900/40 text-center space-y-0.5">
                  <span className="text-[9px] font-bold text-emerald-700 dark:text-emerald-400 uppercase">Earned</span>
                  <p className="font-black text-emerald-900 dark:text-white text-sm">{leaveBalance.paid}</p>
                </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass-card p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-slate-900 flex flex-col justify-between space-y-4 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-105 dark:border-slate-800/80 pb-3">
                <div className="space-y-0.5">
                  <h3 className="font-extrabold text-sm text-slate-900 dark:text-white flex items-center gap-2">
                    <FileText className="w-5 h-5 text-brand-600 dark:text-brand-400" /> Attendance History
                  </h3>
                  <p className="text-[10px] text-slate-400">View personal daily registers and search records</p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Filter by Date</label>
                  <input
                    type="date"
                      ...
                      <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 text-slate-855 dark:text-slate-200">
                        ...
                        <td className="py-2.5 px-4">
                          <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${
                            item.status === 'Present' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' :
                            item.status === 'Late' ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' :
                            item.status === 'Leave' ? 'bg-sky-50 text-sky-700 dark:bg-sky-500/10 dark:text-sky-400' :
            ...
            <div className="glass-card p-6 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-slate-900 space-y-4 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-105 dark:border-slate-800/80 pb-3">
                <div className="space-y-0.5">
                  <h3 className="font-extrabold text-sm text-slate-900 dark:text-white flex items-center gap-2">
                    <ShieldAlert className="w-5 h-5 text-amber-500" /> Attendance Requests
                  </h3>
                  <p className="text-[10px] text-slate-400 font-medium">Request correction logs for missed check-in or checkout scanners</p>
                </div>
                      ...
                      <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 text-slate-855 dark:text-slate-200">
                        ...
                        <td className="py-2.5 px-4">
                          <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${
                            ...
                            req.status === 'Pending' ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400' :
                            'bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400'
                          }`}>
                            {req.status}
                          </span>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-200 text-xs">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-1.5">
                  <Calendar className="w-5 h-5 text-purple-600" /> Apply for Leave
                </h3>
                <button 
              ...
              <form onSubmit={handleApplyLeaveSubmit} className="space-y-4">
                <div>
                  ...
                  >
                    <option value="Casual">Casual Leave ({leaveBalance.casual} left)</option>
                    <option value="Sick">Sick Leave ({leaveBalance.sick} left)</option>
                    <option value="Paid">Earned Leave ({leaveBalance.paid} left)</option>
                  </select>
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Reason for Leave</label>
                  <textarea
                    value={leaveReason}
        ...
        {showCorrectionModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-200 text-xs">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 className="text-base font-black text-slate-900 dark:text-white flex items-center gap-1.5">
                  <ShieldAlert className="w-5 h-5 text-amber-500" /> Attendance Correction
                </h3>
                <button 
              ...
              <form onSubmit={handleCorrectionSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Actual Time</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Reason for correction</label>
                  <textarea
          ...
          <div className="p-4 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              ...
              <div className="p-3 rounded-2xl bg-sky-50 dark:bg-sky-950/40 border border-sky-200 dark:border-sky-900 text-center">
                <p className="text-[10px] font-bold text-sky-700 dark:text-sky-400 uppercase">On Leave</p>
                <p className="text-lg font-black text-sky-800 dark:text-sky-300 mt-0.5">{liveSummaryStats.leave}</p>
              </div>
                          ...
                          <td className="py-3 px-4 text-center">
                            <div className="flex items-center justify-center gap-1">
                              {(['Present', 'Absent', 'HalfDay', 'Leave'] as const).map(st => {
                                ...
                                else if (st === 'Leave') activeStyle = 'bg-sky-600 text-white font-black border-sky-600 shadow-sm';
                              ...
                              {approvedLeave && (
                                <div className="flex items-center gap-1 ml-1.5">
                                  ...
                                  {canOverrideLeave && (
                                    ...
                                    >
                                      {overrideLeaveSet.has(s.id) ? <Unlock className="w-3.5 h-3.5 text-rose-500" /> : <Lock className="w-3.5 h-3.5 text-sky-600" />}
                                    </button>
                                  )}
                                </div>
                              )}
          ...
          <div className="glass-card rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-900">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 text-[10px] font-bold">
                ...
                <span className="px-2 py-0.5 rounded bg-sky-100 text-sky-800">L = Leave</span>
              ...
              <table className="w-full text-left border-collapse text-[11px]">
                <thead>
                  <tr className="bg-slate-100/70 dark:bg-slate-800/60 text-slate-500 font-bold uppercase border-b">
                    ...
                    <th className="py-2.5 px-2 text-center text-sky-600">L</th>
                    <th className="py-2.5 px-2 text-center text-sky-600">%</th>
                  </tr>
                </thead>
                        ...
                        <tr key={s.id} className="hover:bg-slate-50/50">
                          ...
                          {registerDaysArray.map(dayNum => {
                            ...
                            if (record) {
                              ...
                              else if (record.status === 'Leave') { code = 'L'; lCount++; badgeStyle = 'text-sky-700 bg-sky-100 font-bold'; }
